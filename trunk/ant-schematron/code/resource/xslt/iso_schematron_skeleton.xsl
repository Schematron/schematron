...
<!--
   ...
   element <schold:ns> for declaring the namespaces and prefixes used in Xpaths in 
   attribute values; it does not extend the XML Namespaces mechanism: if a name
   in an XPath has a prefix, there must be an <schold:ns> element for that prefix; if
...
<!--
  VERSION INFORMATION
   Version: 2007-10-25
   	 	* RJ Add saxon extensions to support extra typing to XSL2
   	 	* RJ Add support for Xpath2 support 
   Version: 2007-10-17
	...
	* FG better title section using ( @id | schold:title)[last()]
   ...
   Version: 2007-01-18 (First beta candidate for comment)
          ...
          * Add exploratory support for schold:schema[@queryBinding='xpath']
   ...
   Version:2007-01-12 (Pre-beta release to Schematron-love-in maillist)
           ...
           * start change schold:rule to allow attributes, PIs and comments 
   ...
   TODO:
    ...
    KNOWN TYPICAL LIMITATIONS:
      * Don't use <schold:ns prefix="xsl" .../> with a namespace other than the standard
...
<xsl:copy-of select="document( $document-uri,/ )/*"/>
	...
	or (../iso:phase[@id= $phase]/iso:active[@pattern= current()/@id])">
		<xsl:call-template name="process-pattern">
			...
			@id | schold:title returns node-set in document order:
...
<!-- ============================================================== -->
	...
	<!-- Note that process-ns is for reporting. The schold:ns elements are 
	     independently used in the schold:schema template to provide namespace bindings -->
